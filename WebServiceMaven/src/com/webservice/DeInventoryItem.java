...
@Path("/DeInventoryItem")
public class DeInventoryItem {

	@POST
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	@Produces(MediaType.TEXT_HTML)
	public String loginUser(@FormParam("userIDcheck") String userIDcheck,@FormParam("userID") int userID,
			@FormParam("item_code") int item_code,@Context HttpServletResponse response) throws IOException {
	    
		System.out.println("*************************************************");
		System.out.println("inside delete_inventory_item WEBservice");
		...
		if(v.validate(userIDcheck, userID)){
			...
			WebResource webresource= client.resource("https://localhost:8444/MicroServiceMaven/backend/inventoryDelItem/"+output_encrypt+"/"+item_code);
	    ...
	    System.out.println("deleteInventoryRESULT="+myObj.toString());
	    System.out.println("*************************************************");
