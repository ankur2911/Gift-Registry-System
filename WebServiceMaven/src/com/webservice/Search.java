...
public class Search {
	...
	public String loginUser(@FormParam("userIDcheck") String userIDcheck,@FormParam("userID") int userID,
			@FormParam("search") String search,@Context HttpServletResponse response) throws IOException {
	    
		System.out.println("*************************************************");
		System.out.println("inside search WEBservice");
		...
		if(v.validate(userIDcheck, userID)){
			...
			WebResource webresource= client.resource("https://localhost:8444/MicroServiceMaven/backend/search/"+output_encrypt+"/"+search);
	    ...
	    System.out.println("searchRESULT="+myObj.toString());
	    System.out.println("*************************************************");
