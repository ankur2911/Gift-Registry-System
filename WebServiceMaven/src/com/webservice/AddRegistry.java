...
public class AddRegistry {
	...
	public String loginUser(@FormParam("userIDcheck") String userIDcheck,@FormParam("userID") int userID,
			@FormParam("share_status") int shareStatus,@FormParam("share_list") String shareList,@Context HttpServletResponse response) throws IOException {
	    
		System.out.println("*************************************************");
		System.out.println("inside add_registry WEBservice");
		...
		if(v.validate(userIDcheck, userID)){
			...
			WebResource webresource= client.resource("https://localhost:8444/MicroServiceMaven/backend/addRegistry/"+output_encrypt+"/"+shareStatus+"/"+shareList);
