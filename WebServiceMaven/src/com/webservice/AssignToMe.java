...
public class AssignToMe {
	...
	public String loginUser(@FormParam("userIDcheck") String userIDcheck,@FormParam("userID") int userID,@FormParam("username") String username,
			@FormParam("registry_id") int registry_id,@FormParam("item_code") String item_code,@Context HttpServletResponse response) throws IOException {
	    
		System.err.println("inside assign to me WEBservice");
		...
		if(v.validate(userIDcheck, userID)){
			...
			WebResource webresource= client.resource("https://localhost:8444/MicroServiceMaven/backend/assignToMe/"+output_encrypt+"/"+username+"/"+registry_id+"/"+item_code);
